...
export default function EditClass({ cls, onClose, refresh }) {
    ...
    useEffect(() => {
        const fetchOptions = async () => {
            try {
                ...
                setDepartments(
                    (deptRes?.data || []).map((d) => ({ value: d._id, label: `${d.name} (${d.code})` }))
                );
                setCourses(
                    (courseRes?.data || []).map((c) => ({ value: c._id, label: `${c.title} (${c.code})` }))
                );
                setLecturers(
                    (lecRes?.data || []).map((l) => ({ value: l._id, label: `${l.name} (${l.LecID})` }))
                );
            } catch (e) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await API.put(`/classes/${encodeURIComponent(cls.code)}`, payload);
            if (res?.data) {
    ...
    const findOpt = (options, value) => options.find((o) => o.value === value) || null;
            ...
            >
                <div className="p-5 space-y-4">
                    <div className="flex items-center justify-between">
                        ...
                        <button type="button" onClick={onClose} className="hover:cursor-pointer" aria-label="Close">
                            <X size={22} />
                        </button>
                    </div>
                    ...
                    <form className="grid grid-cols-1 md:grid-cols-3 gap-3" onSubmit={handleSubmit}>
                        <div className="flex flex-col md:col-span-3">
                            ...
                            <input name="title" value={formData.title} onChange={handleChange} className="input-field ml-0 p-2" />
                        </div>
                        ...
                        <div className="flex flex-col">
                            ...
                            <Select
                                ...
                                onChange={(opt) => setFormData((p) => ({ ...p, department: opt?.value || "" }))}
                            />
                        </div>
                        <div className="flex flex-col">
                            ...
                            <Select
                                ...
                                onChange={(opt) => setFormData((p) => ({ ...p, course: opt?.value || "" }))}
                            />
                        </div>
                        <div className="flex flex-col">
                            ...
                            <Select
                                ...
                                onChange={(opt) => setFormData((p) => ({ ...p, lecturer: opt?.value || "" }))}
                            />
                        </div>

                        <div className="flex flex-col">
                            <label className="text-sm">Semester</label>
                            <input name="semester" value={formData.semester} onChange={handleChange} className="input-field ml-0 p-2" />
                        </div>
                        <div className="flex flex-col">
                            <label className="text-sm">Year</label>
                            <input name="year" type="number" value={formData.year} onChange={handleChange} className="input-field ml-0 p-2" />
                        </div>
                        <div className="flex flex-col">
                            <label className="text-sm">Room</label>
                            <input name="room" value={formData.room} onChange={handleChange} className="input-field ml-0 p-2" />
                        </div>

                        <div className="flex flex-col md:col-span-2">
                            <label className="text-sm">Schedule</label>
                            <input name="schedule" value={formData.schedule} onChange={handleChange} className="input-field ml-0 p-2" />
                        </div>
                        <div className="flex flex-col">
                            <label className="text-sm">Status</label>
                            <select name="status" value={formData.status} onChange={handleChange} className="input-field ml-0 p-2">
